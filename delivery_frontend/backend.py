...
"""
DATABASEPATH = r"\\192.168.0.10\public\DataBase_Delivery\deliveries.db"
...
def create_tables():
    with sqlite3.connect(DATABASEPATH) as conn:
        ...
        crsr.execute(
            """CREATE TABLE customers (name TEXT, address TEXT, phone TEXT, contact TEXT);""")
        crsr.execute(
            """CREATE TABLE vendors (name TEXT, address TEXT, phone TEXT, contact TEXT);""")
...
def update_docket(docket, tasks):
    with sqlite3.connect(DATABASEPATH) as conn:
        crsr = conn.cursor()
        crsr.execute("""UPDATE deliveries SET tasks=?,completed_tasks=? WHERE docket==?;""",
                     (json.dumps(tasks[0]), 0, docket))
        conn.commit()
...
def submit_partner(data, db_name):
    ...
    with sqlite3.connect(DATABASEPATH) as conn:
        ...
        if db_name == CUSTOMER_DB:
            crsr.execute("""INSERT INTO customers VALUES (?,?,?,?)""",
                         (data["name"], data["address"], data["phone"], data["contact"]))
        elif db_name == VENDOR_DB:
            crsr.execute("""INSERT INTO vendors VALUES (?,?,?,?)""",
                         (data["name"], data["address"], data["phone"], data["contact"]))
        conn.commit()
...
def submit_delivery(data):
    ...
    with sqlite3.connect(DATABASEPATH) as conn:
        ...
        crsr.execute(
            ...
            """,
            (crsr.lastrowid, data["customer"], data["vendor"], data["completed_tasks"], data["date_client"], data["date_request"],
             data["date_shipment"], json.dumps(data["tasks"]), data["note"], data["delivery_address"], data["closed"]))
        conn.commit()
...
def short_get_delivery(docket):
    delivery = None
    with sqlite3.connect(DATABASEPATH) as conn:
        conn.row_factory = generator
        crsr = conn.cursor()
        crsr.execute("""SELECT docket, customer, date_shipment, delivery_address
                    FROM deliveries WHERE docket=? ORDER BY date_shipment DESC;""", (docket,))
        delivery = crsr.fetchone()
    return delivery
...
def get_recent(num=None):
    ...
    with sqlite3.connect(DATABASEPATH) as conn:
        ...
        crsr = conn.cursor()
        crsr.execute("""
                    SELECT docket, customer, date_shipment, delivery_address
                    FROM deliveries ORDER BY date_shipment DESC
                    """)
...
def close_docket(docket):
    with sqlite3.connect(DATABASEPATH) as conn:
        crsr = conn.cursor()
        crsr.execute("""UPDATE deliveries SET completed_tasks=? WHERE docket==?;""",
                     (1, docket))
        conn.commit()
